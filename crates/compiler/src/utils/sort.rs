...
impl Sort {
    pub fn sort_bfs<T>(nodes: &Vec<T>, edges: &Vec<(usize, usize, usize, usize)>) -> Vec<usize> {
        ...
        for &(src, _src_idx, dst, _dst_idx) in edges {
    ...
    pub fn _sort_dfs<T>(nodes: &Vec<T>, edges: &Vec<(usize, usize, usize, usize)>) -> Vec<usize> {
        ...
        for i in 0..nodes.len() {
            if !visited[i] {
                Self::_dfs(i, &mut visited, &adjacent, &mut sorted);
            }
        }
    ...
    fn _dfs(
        ...
        for &u in &adjacent[index] {
            if !visited[u] {
                Self::_dfs(u, visited, adjacent, sorted);
            }
        }
        sorted.push(index);
    }
